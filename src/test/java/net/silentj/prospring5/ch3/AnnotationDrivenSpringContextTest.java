...
class AnnotationDrivenSpringContextTest {
    @Test
    public void shouldCreateRendererAndRenderToSystemOutWithAnnotations() {
        ...
        assertEquals("Hello from annotation-land!\n", output);
        ...
        assertEquals(AnnotatedHelloWorldMessageProvider.class, ctx.getBean(MessageProvider.class).getClass());

        // Make some assertions about the beans to see what's right
        // (This isn't really something we care about... more just exploring)
        assertThat(ctx.getBeanDefinitionNames(), hasItemInArray("annotatedHelloWorldMessageProvider"));
